...
<div class="container">
    ...
    <a href="/write" class="btn btn-outline-warning">게시글 등록 </a>
       ...
       <c:forEach items="${list}" var="list" varStatus="status">
         ...
         <tr>
       <td>${list.no}</td> 
        <td><a href="/one?no=${list.no}">     ${ list.title}    </a></td>  
       <th class="date" data-video-date= ${list.date}><span class="list-date"></span></th> 
              <%--  <td> <fmt:formatDate value="${list.formatDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td> --%>
       </tr>
		...
		<ul class="pagination">
			...
			<li class="page-item ${paging.next ? '' : 'disabled' }"><a class="page-link"
				href="/list?page=${paging.endPage + 1 }">Next</a></li>
		</ul>
