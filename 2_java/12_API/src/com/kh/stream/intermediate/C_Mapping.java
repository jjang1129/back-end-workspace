...
// mapXXX : 요소를 대체하는 요소로 구성된 새로운 스트림 생성 
    ...
    public void method1() {
    	...
    	// 수학 점수의 평균 (average)를 쓰려면 mapToInt 써야함 
    	
    	              students.stream().mapToInt(student-> student.getMath()).average();
    	              
     //  OptionalDouble avg = students.stream().mapToInt(student-> student.getMath()).average();   형태 변환             
    ...
    //flastMapxxx : 하나의 요소를 복수의 요소들로 구성된 새로운 스트림을 생성한다 
     public void method2() {
    	 ...
    	 list.stream()
    	     ...
    	     // .flatMap(str -> Arrays.stream(str.split(" ")))
    	      .forEach(str -> System.out.println(str)); //.forEach(system.out::println);
      ...
      *  -asDoubleSteam : IntStream,LongStream을 DoubleStream으로 변환 
     ...
     public void method3() {
    	 ...
    	 Arrays.stream(array1).asDoubleStream().forEach(num -> System.out.println(num));
    	 
    	 System.out.println();
    	 
    	 
    	 Arrays.stream(array2).boxed().forEach(num -> System.out.println(num));
