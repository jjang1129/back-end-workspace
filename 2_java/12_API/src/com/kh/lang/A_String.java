...
public class A_String {
	...
	public  void method1() {
		...
		System.out.println("str 1 == str 3 : " + (str1 == str3)); // false 
	...
	// String 클래스에서 제공하는 메서드 
     public void method2() {
    	 ...
    	 System.out.println("1. char At  : " + ch);
    	 
    	 //2. concat(String str) : String 
    	 // 전달받은 문자열과 원본 문자열을 하나로 합친 새로운 문자열을 생성해서 리턴 
    	 String str2 = str.concat("!!!!");
    	 str2 = str + "!!!!";
    	 System.out.println("2. cocat : "+str2);
    	 
    	 // 3. substring (int beginIndex ) : String 
    	 //    substring (int beginIndex, int endIndex) : String 
    	 //    문자열의 beginIndex 부터 endIndex -1 까지 (enddIndex가 생략되면 끝까지) 
    	 //    의 문자열을 새로 생성해서 리턴 
    	 ...
    	 //6. toUpperCase() / toLoewrCase() : String 
    	 ...
    	 System.out.println("   toLoewrCase :" +str7);
    	 ...
    	 // 배열을 리스트 (ArrayList)로 변환 반복문을 이용
	 ...
	 public void method3() {
		 ...
		 // 1. append (Stirng str) : StringBuilder 
		 // 기존 문자열 뒤에 문자열 추가 
		   sb.append("Java!");
		   
		  // 2. insert(int offset, String str) : StringBuilder
		   // 문자열의 offset(index) 위치부터 전달 받은 문자열을 추가 ! 
		   sb.insert(1,"eeeee");
		   
		  // 3. delete (int start, int end ) : StringBuilder
		   // start에서 end - 1 까지의 인덱스에 해당하는 문자열 모두 삭제 
		   sb.delete(1, 6);
		  
		   
		  // 4. reverse() : StringBulider 
		  // 문자열의 순서를 역으로 바꾼다 
		   sb.reverse();
	...
	//	  sb2.reverse();
	 ...
	 public void method4() {
		...
		StringTokenizer st = new StringTokenizer(str,","); // 객체 생성 + 전달받은 생성자 str, 구분자 ","
		
		// System.out.println("토큰이 남아 있는가? : "+st.hasMoreTokens());
		// nextToken : 토큰을 하나씩 꺼내옴 
		// System.out.println("nextToken : "+st.nextToken());
		
		// countTokens :  남아있는 토큰의 수를 확인
		System.out.println("과목 개수 : " + st.countTokens()); 
		
		while(st.hasMoreTokens()) {
			
			System.out.println("하나씩 꺼내옴 : " +st.nextToken());
