...
public class C_sorting {
	...
	/*
	 * 삽입정렬(insertion sort) : 동작원리는 직관적, 선택 정렬에 비해서는 구현 난이도가 높아짐 
	 * - 정렬 알고리즘에서 가장 간단하고 기본이 되는 알고리즘
	 * - 필요할 때만 위치를 바꾸므고 '데이터가 거의 정렬되어 있을 때 ' 효율적 
	 * 
	 * 
	 * 
	 * */
	...
	/*
	 * 버블 정렬 
	 * - 인접한 두 수를 비교해서 큰 수를 뒤로 보내는것이 특징
	 *  - 구현이 쉽다는 장점이 있지만, 다른 정렬에 비해 정렬 속도가 느리며 .
	 *  역순으로 정렬할때 가장 느린속도를 가짐
	 * */
	public void method6() {		
		int [] arr = {7,5,9,0,3,1,6,2,4,8};
