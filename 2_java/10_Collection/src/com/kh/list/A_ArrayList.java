...
//		list.add(2);
	...
	public void method2() {
		...
		// 4. size : 리스트 안에 몇개의 데이터가 있는지 볼수있는애 
		System.out.println("사람 수 : "+ list.size());
		
		
		// 5. remove : 해당 인덱스의 객체 삭제
		// -> 알아서 크기가 줄어들고 뒤에 객체가 앞으로 다 떙겨온다
		...
		list.subList(0, 2);
		System.out.println(list.subList(0, 2)); // 0<= 인덱스 < 2 , => 0,1 
		System.out.println(list);
	...
	public void method3() {
		...
		Collections.sort(list);
		System.out.println(list);
		// 12. 내림차순 정렬 : 먼저 sort 메서드를 통해서 오름차순 정렬후 reverse 매서드를 호출하여 정렬 
