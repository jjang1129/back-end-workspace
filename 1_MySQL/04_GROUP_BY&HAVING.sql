...
/*
  ...
  4 HAVING     조건식 (그룹 함수를 가지고 기술)  그룹함수가 조건이면 having 아니면 where?
...
SELECT job_code,sum(salary) '총 급여의 합'
FROM employee
GROUP BY job_code
...
SELECT dept_code,IFNULL(sum(bonus),0)
FROM employee
GROUP BY dept_code
...
/*
   ...
   MYSQL에서의 rollup
   컬럼1, 컬럼2, withe rollup
   
   grouping : rollip 이나 cube에 의해 산출된 값이 해당 컬럼의 집합의 산출물이면 0, 아니면 1
   -> 집계된 값인지 , 아닌지 정도만 구분 
...
/*
  ...
  - 여러 개의 쿼리문에서 조회하려고 하는 컬럼의 개수와 이름이 같아야 사용할 수 있다.
   ...
   주 의 ORDER BY 절은 마지막에 넣어야 사용 가능
   
   UNION (합집함) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출 (중복되는 행 제거) 중복행 제거라 쿼리문 1개에 or 연산자로 대체 가능 
   UNION ALL (합집합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출( 중복행 제거 x) 
   INTERSECT ( 교집합) : 두 쿼리문을 수행한 결과값에 중복된 결과값만 추출 ( MY SQL X)
   MINUS ( 차집합) : 선행 쿼리문의 결과값에서 후행 쿼리문의 결과값을 뺀 나머지 결과값만 추출 ( MY SQL x) 
    ----> INTERSECT , MINUS : WHERE 절에서 AND 연산자를 이용해 처리 가능!
*/

-- 1. UNION 
-- (1) 부서코드가 D5인 사원들의 사번, 사원명 ,부서코드. 급여조회 
...
-- 2 UNION ALL
-- 부서코드가 D5이거나 급여가 300만원 초과인 사원들의 사번, 사원명 ,부서코드. 급여조회 
