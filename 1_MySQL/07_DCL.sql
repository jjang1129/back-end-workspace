/*
     보안 및 관리를 위해 root 계정을 직접 사용하지 않는것을 권장
*/
...
/*
    ...
    스키마 (Schema) : 데이터베이스 내에 특정 테이블, 뷰, 프로시저, 함수등을 그룹화 
    
    --> 데이터베이스와 스키마를 같은 의미로 사용을 하지만, 몇몇 DBMS에서는 다르게 해석
		MYSQL에서는 대두분 두 용어를 같이 사용!
...
-- 스키마 삭제
DROP SCHEMA  IF EXISTS sample; -- 있으면 실행해서 삭제 
DROP SCHEMA sample2; -- 없으면 오류뜸
