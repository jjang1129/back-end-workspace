...
/*
   ...
   - SQL 문을 저장하여 필요할 때마다 다시 입력할 필요 없이 
    간단하게 호출해서 사용 가능하게 하는 코드 블록  VIEW는 SELECT문만 저장함 
    (시작을의미)
    DELIMITER // 
...
-- emp_copy테이블의 모든 데이터를 삭제하는 프로시저 생성 
...
-- 방금 만든 salary_calc 함수를 이용해서 연봉이 4000만원이상인 
...
-- 함수 삭제  IF EXISTS( 존재할때만, 삭제를 한다 = 없는데 삭제하면 경고 만약에 IF EXISTS를 안썼는데 함수가 없으면 에러 )
...
/*
   ...
   - 특정 이벤트가 발생할 때 자동으로 실행될 내용을 정의하여 저장
    ...
    ex) 
    - 회원 탈퇴시 기존 회원테이블에 데이터 DELTETE 후 
     곧바로 탈퇴한 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리 
     - 신고 횟수가 일정 수를 넘었을 때 해당 회원을 블랙리스트로 처리 
     - 입출고에 대한 데이터가 기록(INSERT) 될 떄마다 
...
VALUES (800,'서동문','000000-0000000');


-- employee 테이블에서 UPADTE 수행 후 업데이트 실행 메세지를 log에 담는
-- trg_02 생성 
...
-- 1번 상품이 오늘 날짜로 10개 입고 (prodetail )
INSERT INTO prodetail(pcode,amount,status)
VALUES (1,10,'입고');

-- 1번 상품의 재고 수량이 10 증가 (product)
UPDATE product 
SET stock= stock+10
WHERE pcode=1;
...
/*
  prodetail에 INSET 발생시 
  product에 매번 자동으로 재고수량 update 되도록 정의 
