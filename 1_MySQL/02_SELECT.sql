/*
   ...
   - DBMS : Data Management System의 약자, 데이터베이스 관리 시스템
   ...
   - SQL 종류
	...
	3. DCL(Data Control Language) : 데이터 제어어
       -DBMS에 대한 보안, 무결성, 복구 등 DMBS를 제어하기 위한 언어 
...
/*
  ...
  - SELECT를 통해서 조회된 결과를 RESULT SET이라고 한다.(즉,조회된 행들의 집함)
  - 조회하고자 하는 컬럼들은 반드시 FROM절에 기술한 테이블에 존재하는 컬럼이어야한다.
  - 모든 컬럼을 조회할 경우 컬럼명 대신 * 기호 사용
*/
-- employee 테이블에 전체 사원의 모든 컬럼(*) 정보조회
SELECT *
FROM employee;
...
-- OR (또는), AND(그리고) 
-- 부서코드가 'D6'이거나 급여가 300만원 이상인 사람들의 사원명, 부서코드, 급여 조회 
SELECT emp_name,dept_code, salary
FROM employee
...
/*
   ...
   '%' : 0글자 이상 
   비교대상 컬럼 LIKE '문자%' =>비교대상컬럼값이 문자로 "시작"되는걸 조회
   비교대상 컬럼 LIKE '%문자' =>비교대상컬럼값이 문자로 "끝"나는걸 조회 
   비교대상 컬럼 LIKE '%문자%' =>비교대상컬럼값이 문자가 "포함"되는걸 조회 (키워드 검색)
   
   '_' : 1글자
   비교대상컬럼 LIKE '_문자'=> 비교대상컬럼값에 문자앞에 무조건 한글자가 올 경우 조회
   비교대상컬럼 LIKE '__문자'=> 비교대상컬럼값에 문자앞에 무조건 두글자가 올 경우 조회
   비교대상컬럼 LIKE '_문자_' => 비교대상컬럼값에 문자 앞뒤로 무조건 한글자씩 올 경우 조회
*/
...
-- 이름중에 '하'가 포함된 사원들의 사원명, 주민번호(emp_no), 전화번호(phone) 조회 
SELECT emp_name,emp_no,phone
FROM employee
WHERE emp_name LIKE '%하%';

-- 이름의 가운데 글자가 '하'인 사원들의 사원명, 전화번호 조회( 3글자)
SELECT emp_name,phone
FROM employee
WHERE emp_name LIKE '_하_';


-- 전화번호의 3번째 자리가 1인 사원들의 사번(emd_id), 사원명 ,전화번호, 이메일(email) 조회 
...
*/
-- 부서코드가 D5,D6,D8인 부서원들의 이름, 부서코드 , 금여조회 
SELECT emp_name,dept_code,salary
FROM employee
...
/*
   ...
   -데이터 양을 제한하고 할 때 유용 (페이징 처리)
*/

-- 연봉이 높은 5명의 사원의 사원명, 급여조회
--  LIMIT 절은 두 개의 값이 있을 수 있음!
-- 첫번째 값은 offset, 0부터 시작 시작 행을 지정
...
-- 3. 연봉이 300만원 이상이고 보너스를 받지 않은 사원들의
-- 사번,사원명,급여,보너스 조회 
SELECT emp_id,emp_name,salary*12,bonus
FROM employee
WHERE bonus IS NULL AND salary*12 >= 40000000;

-- 4. 입사일이 '1995-01-01'이상이고 부서배치를 받은 사원들의
-- 사번,사원명,입사일,부서코드 조회 (정렬은 먼저 입사한 기준)
