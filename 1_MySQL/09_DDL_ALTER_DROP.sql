...
/*
  ...
  - 기본값 변경 : ALTER TABLE 테이블명 MODIFY 컬렴명 자료형 DEFAULT 변경할 기본값;
...
DESC dept_copy;
ALTER TABLE dept_copy MODIFY dept_id INT;
ALTER TABLE dept_copy MODIFY cname INT; -- 기존데이터가 없으면 자료형 변경 가능 
...
-- 다중 수정 가능!
-- dept_title 자료형 VARCHAR(20)
-- location_id VARCHAR(2)
-- lname 컬럼 기본값 미국 자료형 VARCHAR (20)
SELECT *
FROM dept_copy;
...
/*
   ...
   ALTER TABLE 테이블명 RANAME COLUMN 기존 컬럼명 TO 변경할컬럼명 ;
*/

-- dept_copy 테이블에서 lname 컬럼을 location_name으로 변경
...
/*
    ...
    - NOT NULL : MODIFY 컬러명 [CONSTRAINT 제약조건명] NOT NULL ;
*/
...
-- dept_copy 테이블의 dept_id 컬럼에 기본키 제약조건 추가 
...
/*
  2-2. 제약조건 삭제
  - NOT NULL : ALTER TABLE 테이블명 MODIFY 컬러명 NULL ;
...
-- 제약조건명을 안걸고 제약을 걸면 제약조건명이 자동으로 설정되는데 그걸 확인하는법
