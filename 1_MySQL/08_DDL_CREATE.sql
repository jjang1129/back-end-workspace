/*
    ...
    - 객체(object)/스키마(Schema)를 
    만들고 (CREATE), 변경하고 (ALTER), 삭제(DROP)하는 언어
  - 즉, 실제 데티어 값이 아닌 구조 자체를 정의하는 언어
...
/*
    ...
    - 종류 : NOT NULL, UNIQUE, CHECK ,PRIMAY KEY , FOREIGN KEY
*/    
...
/*
  ...
  - 추가/수정 시 기존에 있는 데이터값 중 중복값이 있을경우 오류 발생
*/
...
INSERT INTO mem_unique VALUES(2,'user01','pass01','주준영','남',null,null,null);
-- >> unique 제약조건에 위배되어 insert 실패!
INSERT INTO mem_unique VALUES(2,'user02','pass01','주준영','ㄴ',null,null,null);
...
CREATE TABLE mem_check(
  mem_no INT NOT NULL,
   ...
   CONSTRAINT mem_ganer_not_null CHECK(gender IS NOT NULL)
);
SELECT *
FROM mem_check;
...
INSERT INTO mem_check
values( 2,'user02','pass02','이동호','s',null,null,null);
-- >> check 제약조건에 위배되었기 때문에 오류 발생!
INSERT INTO mem_check
values( 2,'user02','pass02','이동호',null,null,null,null);
-- >> cehck 제약조건으로 not null 조건또한 위배되었음을 확인!
INSERT INTO mem_check
values( 1,'user02','pass02','이동호','남',null,null,null);
-- >> 회원번호(mem_no)가 동일해도 insert가 되는게 문mem_nomem_id제!

/*
   PRIMARY KEY(기본키)mem_unique
   - 테이블에서 각 행들을 식별하기 위해 사용될 컬럼에 부여하는 제약조건(식별자 역할) 
   ex) 회원번호, 학번, 사원번호, 부서코드, 직급코드, 제품번호, 주문번호,운송장번호,.....
   - PRIMARY KEY 제약조건을 부여하면 그 컬럼에 자동으로 NOT NULL + UNIQUE 조건이 설정된다
   - 한 테이블 당 오로지 한 개만 설정 
   --> 간혹 복합키 설정도 가능 (거의 쓸일은 없음)
...
INSERT INTO pro_like VALUES( 1, 'A',current_date());
-- >> 복합키인 mem_no 와 product_name 두개가 중복이여야만 오류 발생 
...
-- mem_grade 에서 grade_code가 3인 등급 삭제 
DELETE FROM mem_grade 
WHERE grade_code = 3;
...
/*
    자식 테이블에서 생성시 외래키 제약조건을 부여할 때 삭제옵션 지정 가능 
    ...
    3. ON DELETE CASCADE 
	    : 부모데이터 삭제시 해당 데이터를 쓰고 있는 자식 데이터도 삭제 
*/
DROP TABLE member;
...
DELETE FROM mem_grade
WHERE grade_code = 1;
-- >> 삭제 O, 단 1을 가져다 쓰고 있는 자식 데이터 값이 NULL로 변경 
...
INSERT member(mem_name) VALUES('김현호');
...
-- 테이블만 생성하고 데이터 값은 복사하지않는 방법 

CREATE TABLE employee_copy
SELECT * FROM kh.employee WHERE 1 = 0;
-- 모든 행에 대해서 매번 false 이기 때문에 테이블 '구조'만 복사 
